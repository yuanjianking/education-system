    ...
    public function index($request, $integrated = null)
    {
        ...
        foreach ($cartList as $key => $cart) {
            ...
            if (!empty($cart['goods_option'])) {
                ...
                if ($cart['goods']['plugin_id'] != 44 &&$cart['goods_option']['stock'] <= 0) {
                    $cartList[$key]['goods']['stock_status'] = 1; //库存不足
                }

            } else {
                if ( $cart['goods']['plugin_id'] != 44 && $cart['goods']['stock'] <= 0) {
                    $cartList[$key]['goods']['stock_status'] = 1; //库存不足
                }
            }

            if ($cart['goods']['plugin_id'] != 44 && $cart['goods']['status'] != 1) {
                $cartList[$key]['goods']['stock_status'] = 2; //已下架
            }

            if ($cart['goods']['plugin_id'] != 44 && !empty($cart['goods']['deleted_at'])) {
