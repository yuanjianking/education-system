                ...
                if( $type == 1 ){
                    if (empty($member_merge_set['is_merge_save_level'])) {
                        ...
                        if (!empty($memberinfo_model)) {
                            ...
                            //统一
                            $uni_member = MemberUniqueModel::where('member_id', $uid)->first();
                            if (!empty($uni_member)) {
                                MemberUniqueModel::where('member_id', $uid)->update(['member_id' => $mc_uid]);
                            }

                            //小程序
                            $mini_member = MemberMiniAppModel::getFansById($uid);
                            if (!empty($mini_member)) {
                                MemberMiniAppModel::where('member_id', $uid)->update(['member_id' => $mc_uid]);
                            }

                            //app
                            $app_member = MemberWechatModel::getFansById($uid);
                            if (!empty($app_member)) {
                                MemberWechatModel::where('member_id', $uid)->update(['member_id' => $mc_uid]);
                            }

                            //抖音
                            $douyin_member = MemberDouyinModel::getFansById($uid);
                            if (!empty($douyin_member)) {
                                MemberDouyinModel::where('member_id', $uid)->update(['member_id' => $mc_uid]);
                            }

                            //支付宝
                            $alipay_member = \app\common\models\MemberAlipay::where('member_id', $uid)->first();
                            if (!empty($alipay_member)) {
                                \app\common\models\MemberAlipay::where('member_id', $uid)->update(['member_id' => $mc_uid]);
                            }
