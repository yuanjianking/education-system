...
class MemberService
{
    ...
    public function openidLogin($uniacid, $userinfo, $upperMemberId = NULL)
    {
        ...
        if ((!empty($member_model)) && (!empty($fans_mode) && !empty($member_shop_info_model))) {
            ...
            //DB::transaction(function () use (&$member_id, $uniacid, $userinfo, $member_model, $fans_mode, $member_shop_info_model, $upperMemberId){
                ...
                } elseif (empty($member_model) && ($fans_mode->uid || $fans_mode->member_id)) {
                    $member_id = $this->addMcMemberInfo($uniacid, $userinfo);
                    $this->updateFansMember($fans_mode, $member_id, $userinfo);

                    $member_shop_info_model->member_id = $member_id;
    ...
    public function checkMember($UnionidInfo, $fansInfo, $userInfo)
    {
        ...
        if ($UnionidInfo->member_id != $fansInfo->uid) {
            //new
            if (empty($relation_set['is_merge_save_level'])) {
                if ($UnionidInfo->member_id < $fansInfo->uid) {
            ...
            } elseif ($relation_set['is_merge_save_level'] == 2) {
//                $member_fans_1 = McMappingFansModel::getFansById($UnionidInfo->member_id);
//                $member_fans_2 = McMappingFansModel::getFansById($fansInfo->uid);

//                if (empty($member_fans_1) && empty($member_fans_2)) {
                if ($UnionidInfo->member_id < $fansInfo->uid) {
                    $main_member_id    = $UnionidInfo->member_id;
                    $abandon_member_id = $fansInfo->uid;
                } else {
...
//                } elseif(!empty($member_fans_1) && empty($member_fans_2)) {
//                    $main_member_id = $UnionidInfo->member_id;
//                    $abandon_member_id = $fansInfo->uid;
//                } else {
//                    $main_member_id = $fansInfo->uid;
//                    $abandon_member_id = $UnionidInfo->member_id;
//                }
            } elseif ($relation_set['is_merge_save_level'] == 3) {
                $member_mini_1 = MemberMiniAppModel::getFansById($UnionidInfo->member_id);
                $member_mini_2 = MemberMiniAppModel::getFansById($fansInfo->uid);

                if (empty($member_mini_1) && empty($member_mini_2)) {
                    if ($UnionidInfo->member_id < $fansInfo->uid) {
                        $main_member_id    = $UnionidInfo->member_id;
                        $abandon_member_id = $fansInfo->uid;
                    } else {
                ...
                } elseif(!empty($member_mini_1) && empty($member_mini_2)) {
                    $main_member_id = $UnionidInfo->member_id;
                    $abandon_member_id = $fansInfo->uid;
                } else {
                    $main_member_id = $fansInfo->uid;
                ...
                $member_app_1 = MemberWechatModel::getFansById($UnionidInfo->member_id);
                $member_app_2 = MemberWechatModel::getFansById($fansInfo->uid);

                if (empty($member_app_1) && empty($member_app_2)) {
                    if ($UnionidInfo->member_id < $fansInfo->uid) {
                        $main_member_id    = $UnionidInfo->member_id;
                        $abandon_member_id = $fansInfo->uid;
                    } else {
                ...
                } elseif(!empty($member_app_1) && empty($member_app_2)) {
                    $main_member_id = $UnionidInfo->member_id;
                    $abandon_member_id = $fansInfo->uid;
                } else {
                    $main_member_id = $fansInfo->uid;
...
//            }
            ...
            if (!empty($abandon_member) && $relation_set['is_member_merge'] != 1 && time() > ($abandon_member->createtime + 5*60)) {
                $yz_member_1 = MemberShopInfo::getMemberShopInfo($main_member_id);
                $yz_member_2 = MemberShopInfo::getMemberShopInfo($abandon_member_id);

                if (!empty($yz_member_1)) {
                    MemberShopInfo::uniacid()->where('member_id', $main_member_id)->update(['is_old' => 1, 'mark_member_id' => $abandon_member_id]);
                }
                if (!empty($yz_member_2)) {
                    MemberShopInfo::uniacid()->where('member_id', $abandon_member_id)->update(['is_old' => 1, 'mark_member_id' => $main_member_id]);
                }

                throw new ShopException('会员数据异常，请联系客服');
            } else {
                //小程序
                ...
                //公众号
                $fans = McMappingFans::where('uid', $abandon_member_id)->first();

                if (!is_null($fans)) {
                    McMappingFans::where('uid', $fans->uid)->update(['uid'=>$main_member_id]);
                }
