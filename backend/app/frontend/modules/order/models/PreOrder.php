...
class PreOrder extends Order
{
    ...
    public function setOrderAddress()
    {
        ...
        $dispatchTypes = \app\common\modules\shop\ShopConfig::current()->get('shop-foundation.order-delivery-method');

        foreach ($dispatchTypes as $dispatchType){
            $dispatchType = new $dispatchType();
            if($this->dispatch_type_id == $dispatchType->getId()){
                $orderAddress = $dispatchType->preOrderAddress();
                break;
            }
        }
    ...
    public function isNeedAddress()
    {
        ...
        if (!$this->hasOneDispatchType->need_send) {
            return false;
        }
