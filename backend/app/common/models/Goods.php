...
class Goods extends BaseModel
{
    ...
    {
        $pluginIds = [0,32,33,40,41,44,52,53,92,103];//todo 这些都要写到对应的插件里
            ...
            switch ($key) {
                ...
                case 'keyword':
                    ...
                    if (empty($splice_word)) {
                        $splice_search = 'title like "%' . $value . '%"';
                    } else {
                        $splice_search = '';
                        $splice_array = explode(',', $splice_word);

                        foreach ($splice_array as $val) {
                            $splice_search .= 'title like "%' . $val . '%" and ';
                        }
...
//            }
    ...
    public function getNextLevelPriceAttribute()
    {
        ...
        if ($nextLevel) {
            ...
            return sprintf('%.2f', $this->deal_price - $goods->getNextAmount($this->deal_price, $nextLevel));
