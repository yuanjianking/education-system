...
class ResetpwdController extends BaseController
{
	...
	public function changePwd()
	{
        ...
        {
            if(!preg_match('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!~@#$%^&*?\(\)]).{8,18}$/', $pwd))
            {
    ...
    public function sendSmsV2($mobile, $code, $state, $templateType = 'reg', $sms_type=2)
    {
        ...
        if ($sms['type'] == 1) {
            ...
            $templateCodeLogin = trim($sms['templateCodeLogin']);
            config([
            ...
            }elseif (!is_null($sms_type) && $sms_type == 'login'){
                $issendsms = $sms->send($mobile, $name, $content, $templateCodeLogin);
            }else{
            ...
            if (!is_null($sms_type) && $sms_type == 2) {
                ...
                $response = $aly_sms->sendSms(
                    $sms['aly_signname'], // 短信签名
                    $sms['aly_templateCodeLogin'], // 找回密码短信模板编号
                    $mobile, // 短信接收者
                    Array(  // 短信模板中字段的值
                        "number" => $code
                    )
