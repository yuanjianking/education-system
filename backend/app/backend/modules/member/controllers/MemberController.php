...
class MemberController extends BaseController
{
    ...
    public function index()
    {
        ...
        $phone_count = Member::uniacid()->select(['uid', 'mobile'])
        ...
        $uni_count = Member::uniacid()->select(['uid', 'mobile'])
            ->whereHas('yzMember', function ($q) {
                $q->whereNull('deleted_at');
            })
            ->leftJoin('yz_member_del_log', 'mc_members.uid', '=', 'yz_member_del_log.member_id')
            ->whereNull('yz_member_del_log.member_id')
            ->leftJoin('yz_member_unique', 'mc_members.uid', '=', 'yz_member_unique.member_id')
            ->where('yz_member_unique.uniacid', $uniacid)
            ->count();

        $fans_count = Member::uniacid()->select(['uid', 'mobile'])
            ->whereHas('yzMember', function ($q) {
                $q->whereNull('deleted_at');
            })
            ->leftJoin('yz_member_del_log', 'mc_members.uid', '=', 'yz_member_del_log.member_id')
            ->whereNull('yz_member_del_log.member_id')
            ->leftJoin('mc_mapping_fans', 'mc_members.uid', '=', 'mc_mapping_fans.uid')
            ->where('mc_mapping_fans.uniacid', $uniacid)
            ->count();

        $mini_count = Member::uniacid()->select(['uid', 'mobile'])
            ->whereHas('yzMember', function ($q) {
                $q->whereNull('deleted_at');
            })
            ->leftJoin('yz_member_del_log', 'mc_members.uid', '=', 'yz_member_del_log.member_id')
            ->whereNull('yz_member_del_log.member_id')
            ->leftJoin('yz_member_mini_app', 'mc_members.uid', '=', 'yz_member_mini_app.member_id')
            ->where('yz_member_mini_app.uniacid', $uniacid)
            ->count();

        $app_count = Member::uniacid()->select(['uid', 'mobile'])
            ->whereHas('yzMember', function ($q) {
                $q->whereNull('deleted_at');
            })
            ->leftJoin('yz_member_del_log', 'mc_members.uid', '=', 'yz_member_del_log.member_id')
        ...
        $list['uni_count'] = $uni_count;
        $list['fans_count'] = $fans_count;
...
//        $num = count($member->hasManyParentTeam);
    ...
    public function memberMerge()
    {
        ...
        Member::where('uid', $yz_member->mark_member_id)->delete();
        MemberShopInfo::where('member_id', $yz_member->mark_member_id)->delete();
