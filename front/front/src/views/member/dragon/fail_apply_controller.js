...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      let that = this;
      if (this.fun.isTextEmpty(this.formData.busFullNm)) {
        Toast("请输入企业全称");
        return;
      }
      ...
      if (this.formData.registrant == "1") {
        var json = Object.assign({}, this.formData);
      }
      ...
      axios
        ...
        .then(response => {
          if (response.data.result === 1) {
            if (!this.formData.bussLicense) {
              this.formData.bussLicense = {};
              this.formData.bussLicense.bussLicenseID = response.data.data.id;
              this.formData.bussLicense.bussLicenseURL = response.data.data.url;
            } else {
              this.formData.bussLicense.bussLicenseID = response.data.data.id;
              this.formData.bussLicense.bussLicenseURL = response.data.data.url;
            }
    ...
    idUpRead(e) {
      ...
      axios
        ...
        .then(response => {
          if (response.data.result === 1) {
            if (!this.formData.legalFront) {
              this.formData.legalFront = {};
              this.formData.legalFront.legalFrontPic = response.data.data.id;
              this.formData.legalFront.legalFrontURL = response.data.data.url;
            } else {
              this.formData.legalFront.legalFrontPic = response.data.data.id;
              this.formData.legalFront.legalFrontURL = response.data.data.url;
            }
    ...
    idDownRead(e) {
      ...
      axios
        ...
        .then(response => {
          if (response.data.result === 1) {
            if (!this.formData.legalBack) {
              this.formData.legalBack = {};
              this.formData.legalBack.legalBackPic = response.data.data.id;
              this.formData.legalBack.legalBackURL = response.data.data.url;
            } else {
              this.formData.legalBack.legalBackPic = response.data.data.id;
              this.formData.legalBack.legalBackURL = response.data.data.url;
            }
        ...
        .then(response => {
          if (response.data.result === 1) {
            if (!this.formData.cert) {
              this.formData.cert = {};
              this.formData.cert.certPic = response.data.data.id;
              this.formData.cert.certURL = response.data.data.url;
            } else {
              this.formData.cert.certPic = response.data.data.id;
              this.formData.cert.certURL = response.data.data.url;
            }
        ...
        .then(response => {
          if (response.data.result === 1) {
            if (!this.formData.shop) {
              this.formData.shop = {};
              this.formData.shop.shopPic = response.data.data.id;
              this.formData.shop.shopURL = response.data.data.url;
            } else {
              this.formData.shop.shopPic = response.data.data.id;
              this.formData.shop.shopURL = response.data.data.url;
            }
    ...
    cooperationRead(e) {
      ...
      axios
        ...
        .then(response => {
          if (response.data.result === 1) {
            if (!this.formData.cont) {
              this.formData.cont = {};
              this.formData.cont.contPic1 = response.data.data.id;
              this.formData.cont.contURL = response.data.data.url;
            } else {
              this.formData.cont.contPic1 = response.data.data.id;
              this.formData.cont.contURL = response.data.data.url;
            }
