<template>
  <div id="detailed">
    <c-title :hide="false"
             :text="orderType ==='store'?'我的提成':`我的${this.fun.initWithdrawal()}`"></c-title>
    ...
    <mt-navbar v-model="selected">
      ...
      <mt-tab-item id="0"
                   @click.native="swichTabTItem">{{orderType !=='store'?'申请中':'未结算'}}
      </mt-tab-item>
      <mt-tab-item id="1"
                   @click.native="swichTabTItem">{{orderType !=='store'?'待打款':'已结算'}}
      </mt-tab-item>
      <mt-tab-item id="2"
                   v-if="orderType !=='store'"
                   @click.native="swichTabTItem">已打款
      </mt-tab-item>
      <mt-tab-item id="-1" @click.native="swichTabTItem">{{orderType =='supplier'?'驳回':'已失效'}}
      </mt-tab-item>
    </mt-navbar>
