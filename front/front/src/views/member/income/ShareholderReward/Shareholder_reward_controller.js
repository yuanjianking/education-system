...
export default {
  ...
  methods: {
    ...
    getStatistic() {
      let that = this;
      $http
        .get("plugin.partner-reward.frontend.controllers.reward-log.index")
        .then(
          response => {
    ...
    getData() {
      ...
      $http
        .get(
          "plugin.partner-reward.frontend.controllers.reward-log.getLogByType",
          { page: 1, reward_type: that.rewards_type }
        )
        .then(
          response => {
            if (response.result === 1) {
