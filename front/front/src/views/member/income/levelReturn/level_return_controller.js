...
export default {
  ...
  methods: {
    ...
    initInfo() {
      ...
      $http
        .get(
          "plugin.level-return.api.level-return.get-level-return-statistic",
          {}
        )
        .then(
          function(response) {
    ...
    getData(type) {
      ...
      $http
        .get(
          "plugin.level-return.api.level-return.get-return-log",
          json,
          "获取中"
        )
        .then(
          function(response) {
            that.is_days = response.data.is_days;
            if (response.result == 1) {
          ...
          .then(
            function(response) {
              if (response.result == 1) {
                ...
                switch (type) {
                  case "":
                    that.first_content = that.first_content.concat(
                      response.data.data
                    );
                    break;
                  case "0":
                    that.second_content = that.second_content.concat(
                      response.data.data
                    );
                    break;
                  case "1":
                    that.third_content = that.third_content.concat(
                      response.data.data
                    );
                    break;
                  case "-1":
                    that.fourth_content = that.fourth_content.concat(
                      response.data.data
                    );
                    break;
