...
const start = function() {
  if (tag && self.passWordTest) {
    ...
    wx.startRecord({
      ...
      fail: function(error) {
        self.warntexts = "录音失败";
        MessageBox.alert("微信录音失败" + JSON.stringify(error));
        // alert("微信录音失败" + JSON.stringify(error));
        tag = true;
      },
      cancel: function() {
        self.warntexts = "用户拒绝授权录音";
        MessageBox.alert("用户拒绝授权录音");
        // alert("用户拒绝授权录音");
        tag = true;
      }
    });
...
export default {
  ...
  methods: {
    getData(_isupdata) {
      ...
      $http
        .get(
          "plugin.redpack.frontend.activity.getInfo",
          { aid: that.aid, parent_id: that.parent_id },
          ""
        )
        .then(
          function(response) {
            if (response.result === 1) {
              ...
              if (_isupdata) {
                ...
                if (
                  response.data.activity_info.default_challenge_number ==
                  response.data.challenge_number_total
                ) {
                  // that.$nextTick(() => {
    ...
    getActivityList() {
      ...
      let that = this;
      $http
        .get("plugin.redpack.frontend.activity.getActivityList", {}, "")
        .then(
          function(response) {
            endtag = true;
            if (response.result === 1) {
    ...
    smokePaddword() {
      ...
      if (num > self.info.challenge_number_total) {
      ...
      if (this.isFirstNewClass) {
        setTimeout(() => {
          ...
          longPressing = new Longtap(
            document.getElementById("longpressBtn"),
            800
          );
      ...
      $http
        .get(
          "plugin.redpack.frontend.activity.extractWord",
          { aid: this.aid },
          ""
        )
        .then(
    ...
    translateVoice(localId) {
      ...
      wx.translateVoice({
        ...
        fail: function(error) {
          MessageBox.alert("翻译出错啦，检测不到您的声音或者录音时间太短了");
    ...
    alterAction() {
      ...
      $http
        .post(
          "plugin.redpack.frontend.activity.verifyMobile",
          this.form,
          "正在绑定手机.."
        )
        .then(
    ...
    share(data) {
      ...
      wx.ready(function() {
        ...
        _title = that.fun.isTextEmpty(that.activity_info.share_title)
          ? "标题"
          : that.activity_info.share_title;
        _imgUrl = that.fun.isTextEmpty(that.activity_info.share_thumb)
          ? ""
          : that.activity_info.share_thumb;
        _desc = that.fun.isTextEmpty(that.activity_info.share_content)
          ? "描述"
          : that.activity_info.share_content;
