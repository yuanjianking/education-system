...
<template>
  <div id="redEnvelope"
       :style="{ backgroundColor: activity_info.bg_color }">
    <div class="top_music">
    ...
    <div class="callBoss"
         @click.stop="showCall = true">
      <!-- 联系商家 -->
      <i class="iconfont icon-service_m"
    ...
    <div class="topMain"
      ...
      <img :src="activity_info.brand_logo ? activity_info.brand_logo : ''"
           alt=""
           class="logo" />
      <div class="Countdown">
        ...
        <div class="time-up">
          ...
          <yd-countdown slot="right"
                        :time="
              activity_info.countdown_time
                ? activity_info.countdown_time[1]
                : '1642610472'
            "
                        timetype="timestamp"
                        class="bottom_time"
                        :callback="timeEnd"
                        v-if="!fun.isTextEmpty(activity_info.countdown_time)">
            <em class="aa">{%d1}<em style="font-weight: bold;">{%d2}</em> </em>
            天
            <em class="aa">{%h1}<em style="font-weight: bold;">{%h2}</em></em>
            时
            <em class="aa">{%m1}<em style="font-weight: bold;">{%m2}</em></em>
            分
            <em class="aa">{%s1}<em style="font-weight: bold;">{%s2}</em></em>
            秒
          </yd-countdown>
        </div>
        <div class="time-down">
    ...
    <div class="operat">
      <div
        class="warnClass"
        :class="[!fun.isTextEmpty(warntexts) ? 'baColors' : '']"
      >
        {{ warntexts }}
      </div>
      <div class="voice-remote-main">
        <div
          class="voice-remote"
          :class="[recording ? 'recordingClass' : '']"
          v-show="recording"
        >
      ...
      <div class="btn-box">
        <router-link
          :to="fun.getUrl('redEnvelopeRecord', { tag: 1, aid: aid })"
          tag="span"
          >邀请记录</router-link
        >
        <router-link
          :to="fun.getUrl('redEnvelopeRecord', { tag: 2, aid: aid })"
          tag="span"
          >挑战记录</router-link
        >
      </div>
      <div class="Password" v-show="passWordTest">
        您的口令：{{ passWordTest }}
      </div>
      <div>
        <div
    ...
    <div class="Introduction">
      <p class="text">
        *邀请{{
          activity_info.invite_people
        }}个好友完成挑战即可增加1次挑战次数！每人限{{
      ...
      <div class="IntroductionText">
        ...
        <div class="rigth-a">
          点击抽取口令按钮，随机抽取口令！<em
            v-if="activity_info.verify_mobile == 1"
            style="color:#de262a;"
            >参加活动需验证手机号！</em
          >
        </div>
      ...
      <div class="IntroductionText">
        ...
        <div class="rigth-a">
          点击喊出口令按钮,长按录音按钮,<em style="color:#de262a;"
            >提示开始录音时</em
          >,对着手机说出抽取到的口令!系统将上传您的口令录音到微信语音识别接口,将语音口令识别为文字,如果识别结果和口令一致，您将获得<em
            style="color:#de262a;"
          >
            {{ activity_info.min_withdraw }}-{{
              activity_info.max_withdraw
            }} </em
    ...
    <div class="templateStyle darebox">
      ...
      <div class="dare-num">
        ...
        <div class="dare-flex-bottom">
          ...
          <div class="text">
            已发奖励({{ activity_info.withdraw_code_name }})
          </div>
          <div class="text">
            剩余奖励({{ activity_info.withdraw_code_name }})
          </div>
        </div>
      </div>
      <div class="dare-title">
        ...
        <div class="Integral text-over">
          累积奖励({{ activity_info.withdraw_code_name }})
        </div>
      </div>
      <div class="main">
    ...
    <div class="templateStyle">
      ...
      <div
        style="padding:0 0.875rem;line-height: 1.375rem;"
        class="clearOl"
        v-html="activity_info.agreement"
      ></div>
    </div>
    <!--垫高-->
      ...
      <div class="verification">
        ...
        <div class="inputBox">
          ...
          /><span
            ...
            v-show="!cationCode"
            >获取验证码</span
          ><span class="getcodeClass" v-show="cationCode"
            >{{ count }}秒后重新获取</span
          >
        </div>
        <div class="bottomBox">
      ...
      <transition name="bounce">
        <div class="redEnvelopePopup" v-if="showredEnvelopes">
          ...
          <div class="logo">
            <img
              :src="
                activity_info.brand_logo
                  ? activity_info.brand_logo
                  : require('../../../assets/images/addcheck.png')
              "
              alt=""
            />
          </div>
...
#redEnvelope {
        ...
        .time-left {
          ...
          ul {
            ...
            li {
              ...
              border-radius: 100%;
              overflow: hidden;
              background: url("../../../assets/images/photo-mr.jpg") no-repeat
                center center;
              background-size: 100%;
              display: inline-block;
              img {
  ...
  .operat {
    margin-top: 3.5rem;
    height: 23.875rem;
    background: url("../../../assets/images/redEnvelope/default_pic@2x.png")
      no-repeat;
    ...
    .voice-remote .icon {
      ...
      top: 0;
      left: 0;
      background: url("../../../assets/images/recordIcon2.png") no-repeat center
        center;
      background-color: #ffffff;
      background-size: 60%;
      border-radius: 50%;
  ...
  .redEnvelopePopup {
    width: 22.625rem;
    height: 31.875rem;
    background: url("../../../assets/images/redEnvelope/reward_bg@2x.png")
      no-repeat center;
    background-size: cover;
    .total {
    ...
    .btn {
      ...
      box-shadow: 0rem 0.344rem 1.188rem 0rem rgba(159, 11, 0, 0.23),
        inset 0.006rem -0.125rem 0.5rem 0rem rgba(255, 228, 201, 0.31);
  ...
  .RecordPopup {
    ...
    .voice-remote .icon {
      ...
      top: 0;
      left: 0;
      background: url("../../../assets/images/recordIcon.png") no-repeat center
        center;
