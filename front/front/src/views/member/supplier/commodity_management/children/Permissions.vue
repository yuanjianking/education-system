  ...
  <div id="Permissions">
      <div class="power_box">
          ...
          <ul class="number_power">
            ...
            <li>
              <span>会员每月限购数量：</span>
              <input type="text" v-model="month_buy_limit" placeholder="请输入会员每月限购数"  id="inputs" @blur="clearSrcoll()">
            </li>
          </ul>
        </div>
    ...
    <!-- <yd-popup v-if="show1" v-model="show1" position="bottom" height="100%" width="100%" :close-on-masker="false" masker-opacity=".7"> -->
      <div class="select_box">
        ...
        <div class="list">
          <yd-checkbox v-model="show_levels_CheckedAll" shape="circle" :change="checkAll" v-if="levelsList.length">全选 </yd-checkbox>
          <yd-checklist color="#f15353" v-model="show_levels" ref="checklistDemo" :callback="change">
            <yd-checklist-item :val="item.id" v-for="(item, index) in levelsList" class="deletaAtfer" :key='index'>
                <div style="height: 50px;line-height: 50px;">{{item.level_name}}</div>
            </yd-checklist-item>
          </yd-checklist>
    ...
    <!-- <yd-popup v-if="show2" v-model="show2" position="bottom" height="90%" width="100%" :close-on-masker="false" masker-opacity=".7"> -->
      <div class="select_box">
        <div class="title">
          <h2>会员等级购买权限</h2>
          <i class="iconfont icon-guanbi" @click="showPopup"></i>
        ...
        <div class="list">
          <yd-checkbox v-model="buy_levels_CheckedAll" shape="circle" :change="checkAllBuyLevels" v-if="levelsList.length">全选 </yd-checkbox>
          <yd-checklist color="#f15353" v-model="buy_levels" ref="checklistCheckAll" :callback="listCheckAllChange">
            <yd-checklist-item :val="item.id" v-for="(item, index) in levelsList" class="deletaAtfer" :key='index'>
                <div style="height: 50px;line-height: 50px;">{{item.level_name}}</div>
            </yd-checklist-item>
          </yd-checklist>
    ...
    <!-- <yd-popup v-if="show3" v-model="show3" position="bottom" height="90%" width="100%" :close-on-masker="false" masker-opacity=".7"> -->
      <div class="select_box">
        <div class="title">
          <h2>会员组浏览权限</h2>
          <i class="iconfont icon-guanbi" @click="showPopup"></i>
        ...
        <div class="list">
          <yd-checkbox v-model="show_groups_CheckedAll" shape="circle" :change="checkAllShowGroups" v-if="groupsList.length">全选 </yd-checkbox>
          <yd-checklist color="#f15353" v-model="show_groups" ref="ShowGroupsCheckAll" :callback="showGroupsChange">
            <yd-checklist-item :val="item.id" v-for="(item, index) in groupsList"  class="deletaAtfer" :key='index'>
                <div style="height: 50px;line-height: 50px;">{{item.group_name}}</div>
            </yd-checklist-item>
          </yd-checklist>
    ...
    <!-- <yd-popup v-if="show4" v-model="show4" position="bottom" height="90%" width="100%" :close-on-masker="false" masker-opacity=".7"> -->
      <div class="select_box">
        <div class="title">
          <h2>会员组购买权限</h2>
          <i class="iconfont icon-guanbi" @click="showPopup"></i>
        ...
        <div class="list">
          <yd-checkbox v-model="buy_groups_CheckedAll" shape="circle" :change="checkAllBuyGroups" v-if="groupsList.length">全选 </yd-checkbox>
          <yd-checklist color="#f15353" v-model="buy_groups" ref="buyGroupsCheckAll" :callback="buyGroupsChange">
            <yd-checklist-item :val="item.id" v-for="(item, index) in groupsList"  class="deletaAtfer" :key='index'>
                <div style="height: 50px;line-height: 50px;">{{item.group_name}}</div>
            </yd-checklist-item>
          </yd-checklist>
  ...
  methods: {
    ...
    getDataInfo() {
      ...
      if(that.isType){
        ...
        that.once_buy_limit = this.DataInfo.privilege.once_buy_limit
        that.total_buy_limit = this.DataInfo.privilege.total_buy_limit
        that.day_buy_limit = this.DataInfo.privilege.day_buy_limit
        that.week_buy_limit = this.DataInfo.privilege.week_buy_limit
        that.month_buy_limit = this.DataInfo.privilege.month_buy_limit
    ...
    //弹窗相关------
    change(value, show_levels_CheckedAll) {
      this.show_levels_CheckedAll = show_levels_CheckedAll;
      let that = this
      //用于弹窗的val 值（id）与 input的level_name对应，ydui组件显示的是val值
      var arr1 = this.levelsList.filter(function(v){
        return that.show_levels.indexOf(v.id)!== -1 // 利用filter方法来遍历是否有相同的元素
      })
      this.hyll_Model = arr1.map(function (user) { return user.level_name; });
    ...
    listCheckAllChange(value, buy_levels_CheckedAll) {
      ...
      this.hygm_Model = arr2.map(function (user) { return user.level_name; });
    },
    showGroupsChange(value, show_groups_CheckedAll) {
      ...
      this.hyzll_Model = arr3.map(function (user) { return user.group_name; });
    },
    buyGroupsChange(value, buy_groups_CheckedAll) {
      ...
      this.hyzgm_Model = arr4.map(function (user) { return user.group_name; });
    },
    //全选 --------------------
    checkAll() {
      this.show_levels_CheckedAll = !this.show_levels_CheckedAll;
      this.$refs.checklistDemo.$emit('ydui.checklist.checkall', this.show_levels_CheckedAll);
    },
    checkAllBuyLevels() {
      this.buy_levels_CheckedAll = !this.buy_levels_CheckedAll;
      this.$refs.checklistCheckAll.$emit('ydui.checklist.checkall', this.buy_levels_CheckedAll);
    },
    checkAllShowGroups(){
      this.show_groups_CheckedAll = !this.show_groups_CheckedAll;
      this.$refs.ShowGroupsCheckAll.$emit('ydui.checklist.checkall', this.show_groups_CheckedAll);
    },
    checkAllBuyGroups(){
      this.buy_groups_CheckedAll = !this.buy_groups_CheckedAll;
      this.$refs.buyGroupsCheckAll.$emit('ydui.checklist.checkall', this.buy_groups_CheckedAll);
    },
