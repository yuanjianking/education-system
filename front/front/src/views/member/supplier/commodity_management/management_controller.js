  ...
  methods: {
    ...
    init() {
      ...
      if (item.status !== 1){
		  ...
		  $http.get('plugin.store-cashier.frontend.store.goods.changeGoodsStatus', {goods_id:item.id,status: status}, "加载中").then( response => {
			  if (response.result === 1) {
      ...
      $http.post(urls, json, "加载中").then(function (response) {
        if (response.result === 1) {
          ...
          if (that.$route.params.store == 'store') {
            ...
            for (var i = 0; i < response.data.list.data.length; i++) {
              response.data.list.data[i].edit_active = false;
            }
