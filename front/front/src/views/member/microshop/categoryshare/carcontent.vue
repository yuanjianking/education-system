<template>
  <div id="carcontent">
    <div class="branch"
         v-if="list.goo&&list.set.cat_adv_img"><a :href="list.set.cat_adv_url|href_filters"><img :src="list.set.cat_adv_img" /></a></div>
    <div class="sote"
         v-if='list.goo'>
      <ul>
    ...
    <template v-for="item in list.data"
              v-if='list.set.cat_level==3'>
      <div class="branch"
           v-if="item.adv_img"><a :href="item.adv_url|href_filters"><img :src="item.adv_img" /></a></div>
      <div class="sote"
           v-if='list.data'>
        <h3 class=""
            v-if='item.name'>
        ...
        <ul v-if="item.has_many_children.length>0">
          <li v-for="(catshop,i) in item.has_many_children"
              :key='i'>
    ...
    <template v-if="!list.goo">
      <template v-if='list.set'>
        <div class="branch"
             v-if='list.set.cat_level==2 && $store.state.category.adv_img'><a :href="$store.state.category.adv_url|href_filters"><img :src="$store.state.category.adv_img" /></a></div>
        <div class="sote"
