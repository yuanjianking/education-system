...
export default {
  ...
  methods: {
    ...
    ordertBtn() {
      ...
      if(Bol){
        MessageBox.alert('入住日期没有住房，请重新选择入住日期');
        return
      }
      let arr=[start];
      ...
      for(let j = 0;j<arr.length;j++){
        ...
        if(bol){
            if(nextPriceArr[index].is_manfang ==1){
              MessageBox.alert('日期'+nextPriceArr[index].date+'已经满房，请重新选择日期');
              return;
            }else{
              picArr.push({
          ...
          MessageBox.alert('入店至离店期间包含没有住房的日期，请重新选择日期');
          return;
        }
      }
      if(picArr.length>0){
