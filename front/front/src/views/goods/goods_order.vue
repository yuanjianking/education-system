<template>
  <div id="goodsinfo"
    ...
    <template v-if="!isShowDistributionStation&&this.$route.params.tag != 'POrder'&&!is_open_package_deliver">
      <div v-show="store_id">
        <mt-navbar v-model="selected">
          <template v-for="item in dispatch">
            <mt-tab-item @click.native="getDataActionStoreCart" :id="String(item)">{{ dispatchName[item] }}
            </mt-tab-item>
          </template>
        </mt-navbar>
        ...
        <mt-tab-container v-model="selected">
          <mt-tab-container-item id="1">
            <div class="addr"
              ...
              <p v-show="realname"
                 style="flex: 1;text-align: left;padding: 0 0.625rem;">
                收件人:{{ realname }} {{ mobile }}
                <br />
                <span>{{ address }}</span>
              </p>
          ...
          <mt-tab-container-item id="2">
            ...
            <div style="height: 0.3125rem;"></div>
            <div v-if="
              store_info.delivery_information == null ||
              store_info.delivery_information == 0
                ? false
                : true
            ">
              <mt-field :label="recipient_name"
                        v-model="linkinfo.name"
                        :placeholder="`请输入${recipient_name}`"></mt-field>
              <mt-field :label="recipient_mobile"
                        v-model="linkinfo.mobile"
                        :placeholder="`请输入${recipient_mobile}`"></mt-field>
            </div>
            <div style="height: 0.3125rem;"></div>
          </mt-tab-container-item>
    ...
    <div v-if="is_open_package_deliver">
      <mt-navbar v-model="selfCarrySelected"
                 v-if="!fun.isTextEmpty(selfCarryType)">
        <template v-for="item in selfCarryType">
          <mt-tab-item :id="item.dispatch_type_id" @click.native="requestByAddress">{{ item.name }}
          </mt-tab-item>
        </template>
      </mt-navbar>
      ...
      <mt-tab-container v-model="selfCarrySelected">
        <mt-tab-container-item id="8">
          <div class="addr"
               v-if="!fun.isTextEmpty(defaultSelfCarry)">
            <i class="fa fa-map-marker"></i>
            <p style="flex: 1;text-align: left;padding: 0 0.625rem;">
            ...
            <span style="border:1px solid #f15353;padding:0 0.25rem;border-radius:0.25rem;color:#f15353
            "
                  @click.stop="replaceZT">更换</span>
          </div>
          <div class="addr"
               v-if="fun.isTextEmpty(defaultSelfCarry)">
            <div v-if="!isQuest_ing">抱歉，您所在地区暂无{{deliverName}}，或<i style="color:#f15353"
                 @click="replaceZT">手动切换位置</i></div>
            <div v-if="isQuest_ing">正在获取您附近{{deliverName}}信息 ...</div>
          </div>

          <div style="height: 0.3125rem;"></div>

          <mt-field label="提货人姓名"
                    v-model="distributionUserName"
                    placeholder="请输入提货人姓名"></mt-field>
          <mt-field label="提货人手机"
                    v-model="distributionUserMobile"
                    placeholder="请输入提货人手机"></mt-field>
    ...
    <template v-if="isShowDistributionStation&&this.DistributionInfo.length>=1">
      ...
      <van-radio-group v-model="DistributionSelect"
                       class="DistributionSelect">
        <van-radio :name="items.code"
                   v-for="(items,i) in DistributionModel.method_name"
                   checked-color="#f15353"
                   @click="changeDistribution"
                   :key='i'>{{items.name}}</van-radio>
      </van-radio-group>
      ...
      <div class="addr"
           @click="showAddress"
           v-if="this.DistributionSelect == '6'">
        <i class="fa fa-map-marker"></i>
        <p v-show="realname">
          收件人:{{ realname }} {{ mobile }}
          <br />
    ...
    <template v-if="this.$route.params.tag == 'POrder'">
      <!-- 挂单商品模块 -->
      <mt-navbar v-model="POChooseType">
        <mt-tab-item :id="items.code"
                     v-for="items in pdOrderType"
                     :key="items.id"
                     @click.native="POChangeType(items.code)">{{items.name}}</mt-tab-item>
      </mt-navbar>
    ...
    <template v-if="isPhoto">
      <p class="imgUploaderTitle">上传图片<i style="color:#7d7d7d">(选择不低于{{min_count}}张，不高于{{max_count}}张)</i>
      </p>
      <div class="imgUploader ">
        ...
        <van-image-preview v-model="show" :images="fileList1" :start-position='imgIndex'>
        </van-image-preview>
    ...
    <p class="imgUploaderTitle" v-if="isPhoto&&storeNo_photo" style="color: #999999;">
    ...
    <div class="goods-detail">
      <div class="title"
           v-if="isRent && !(rent_deposit_free == 0 && rent_free == 0)">
        <yd-icon class="iconfont icon-tishi"
                 custom
                 size="1.125rem"
                 color="#f15353"></yd-icon>
        <span>您还可以免费租{{ rent_free }}件商品!</span>
      </div>
      <div v-for="(item, index) in order_data"
           :key='index'>
          ...
          <div class="goods"
            ...
            <div class="warp"
              ...
              <ul class="inner">
                ...
                <br />
                <li class="red">
                  <yd-icon class="iconfont icon-rent"
                           custom
                           size="0.75rem"
                           color="#ff9500"
                           v-if="!(rent_deposit_free == 0 && rent_free == 0)"></yd-icon>
                  ￥{{ (g.goods_price / g.total).toFixed(2)  }}元/天<span>×{{ g.total }}</span>
                </li>
                <!-- <li>×{{g.total}}</li> -->
              </ul>
              <div class="right">
                ...
                <ul class="rent-choice">
                  <li class="left">
                    <span class="red">租金：￥{{ g.price }}</span>￥{{ g.order_lease_goods[0].lease_price }}元/天
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div class="warp"
              ...
              <div class="price">
                <p>￥{{ (g.price / g.total).toFixed(2) }}</p>
                <p>×{{ g.total }}</p>
              </div>
              <div class="diyFormDiv" v-if="g.diy_form">
                <div class="dfBtn" v-if="!g.diy_form.diyform_data_id" @click="getGoodDFData(g.goods_id,g.diy_form.form_id,null)">填写表单</div>
                <div class="dfBtn revise_dy" v-if="g.diy_form.diyform_data_id" @click="getGoodDFData(g.goods_id,g.diy_form.form_id,g.diy_form.diyform_data_id)">修改表单</div>
              </div>
            </div>
        ...
        <div class="tbs">
          <div class="num list"
            ...
            <div v-if="!(rent_deposit_free == 0 && rent_free == 0)">
              ...
              <div class="right">
                <mt-switch v-model="isOpenRight"
                           @change="rentSelect"></mt-switch>
              </div>
            </div>
          </div>
          <div class="num list">
            ...
            <div class="right">
              <font color="#E84E40">￥{{ item.order_goods_price }}</font>
            </div>
          </div>
          <div class="freight-num list">
            ...
            <div class="right">
              <font color="#E84E40">
                <span> &nbsp;</span>￥{{ item.dispatch_price }}
              </font>
            </div>
          </div>
          <div class="freight-num list"
               v-for="(item,i) in item.order_fees"
               :key='i'>
          ...
          <div class="score list"
               v-if="item.order_deductions">
            <div v-for="(d,i) in item.order_deductions"
                 :key='i'
              ...
              <div class="right">
                <mt-switch v-model="d.checked"
                           @change="discountHandle(item, d)"></mt-switch>
              </div>
            </div>
          </div>

          <div class="score list"
               v-if="item.order_coin_exchanges">
            <div v-for="(d,i) in item.order_coin_exchanges"
                 :key='i'
                 style="width:100%;display:block;clear:both;margin-bottom: 2.1875rem;">
              <div class="left ">
      ...
      <div class="rent-time"
        ...
        <div class="week">
          <yd-button type="hollow"
            ...
            {{ item.term_name }}<br /><span>优惠{{ item.term_discount }}%
            </span></yd-button>
    ...
    <div class="invoice" @click="show1 = true" v-if="isShowInvoice">
      <span>发票</span>
      <span class="type">
        <font>{{ invoicename }}</font>（{{ companyname }}—{{ showInvoiceTitle }}）
      </span>
      <i class="fa fa-angle-right"></i>
    </div>
    ...
    <yd-popup v-model="show1"
              ...
              class="pc_popup">
      <div class="popup_box">
        <h1>发票<i class="iconfont icon-close11"
             @click="closePopup"></i></h1>
        <p>
        ...
        <div class="type_box">
          <h2>发票类型</h2>
          <div class="type">
            <mt-button :class="{ cur: iscur_c }"
                       v-if="electron_status"
                       @click.native="invoice_type">电子发票
            </mt-button>
            <mt-button :class="{ cur: iscur_d }"
                       v-if="papery_status"
                       @click.native="invoice_type">纸质发票
            </mt-button>
          </div>
        </div>
        <div class="type_box">
          ...
          <div class="input_box"
               v-if="person_status">
            <yd-input type="text" placeholder="请输入发票抬头" v-model="invoice_list.call"></yd-input>
          </div>
          <div class="input_box"
               v-if="unit_status">
            <input type="text"
                   placeholder="请填写单位名称"
                   v-model="invoice_list.call" />
            <input type="text"
                   placeholder="请添加纳税人识别号"
                   v-model="invoice_list.company_number" />
          </div>
        </div>
        <div class="btn">
    ...
    <div class="tbs">
      <div class="price list"
           v-for="item in total_items"
           :key="item.code">
        <div class="left ">{{ item.name }}</div>
        <div class="right">
          <font color="#E84E40"><span v-if="item.code == 'total_deduction_price'">-</span>￥{{
            item.amount
            }}</font>
        </div>
      </div>
      ...
      <div class="price list"
           v-for="item in discount_amount_items"
           :key="item.code">
        <div class="left ">{{ item.name }}</div>
        <div class="right">
      ...
      <div>
        ...
        <div class="agreement"
          ...
          <div class="left ">
            <span>我已认真阅读并同意该</span><span @click="showAgreement"
                  style="color:red;">《租赁协议》</span>
          </div>
        </div>

        <div class="agreement"
             v-if="AgreementPay && !isRent">
          <div class="right">
          ...
          <div class="left">
            <span style="color:red;line-height: 1.7rem"
                  @click="showPay">《支付协议》
            </span>
          </div>
        </div>
    ...
    <div class="detail_pay">
      <ul class="total">
        <li class="deposit">
          <small v-if="isRent">(押金￥{{ goodsInfo.total_deposit }}可退)</small>
        </li>
        <li>
          合计：<font style="color:#f15353">￥<span class="span_t">{{ price }}</span></font>
        </li>
      </ul>
    ...
    <div id="AllGood"
      ...
      <div class="add-info">
        ...
        <mt-field label="真实姓名"
                  placeholder="请输入真实姓名"
                  v-model="member_name"></mt-field>
        <mt-field label="身份证号码"
                  placeholder="请输入身份证号码"
                  v-model="member_card"></mt-field>
        <div class="address_addnav1"
             @click="saveInfo">
          <span>确认修改</span>
        </div>

        <div style="height: 5rem;"></div>
        <div class="address_addnav2"
             @click="open">
          <i class="fa fa-info-circle red"></i><span style="font-size: 14px;">使用说明</span>
        </div>
      </div>
    </div>
    <yd-popup v-model="dyFormPopup"
              position="right"
              class="mint-popup-4"
      ...
      <div class="DYFpopHeader">
        <i class="mintui mintui-back"
            @click="dyFormPopup=false"></i>
        <p>{{dyTiitle}}</p>
      </div>
      <c-dyPopup v-on:submitsave="dfsave" :datas='dfData' :form_data_id='activeFormDataID' :form_id='goodDYDormID' :description="dyDescription" :status="true" :thumb="dyThumb" v-if="dyFormPopup"></c-dyPopup>
    </yd-popup>
      ...
      <div class="add-info">
        <div class="coupon-list-info"
          ...
          <div class="checkList"
            ...
            <yd-checkbox v-model="coupon.checked"
                         :disabled="!coupon.valid"
                         checked-color="#f15353"
                         shape="square"
                         :name="coupon"
                         @change.native="selectCoupon($event, coupon)">
            </yd-checkbox>
          </div>
          <div :class="{
    ...
    <mt-popup v-model="popupDistribution"
              position="bottom"
              class="mint-popup-4 distributionContent"
              closeOnClickModal="true">
      <yd-radio-group v-model="DistributionModel"
                      color="#F00">
        <div class="distributionMain">
          <yd-radio :val="item"
                    class="ss"
                    v-for="(item,i) in DistributionInfo"
                    :key='i'>
            <div class="rightFlex">
              <div class="dis_name">
