...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      $http
        .get(
          "plugin.fight-groups.frontend.controllers.team.index",
          { team_id: that.id },
          "加载中"
        )
        .then(
    ...
    initPopView() {
      if (this.goodsInfo.has_option == 1) {
        ...
        this.popPrice = this.info.has_one_group.min_price + "起----"; //设置默认价格
        optionsMaxCount = this.info.has_one_group.has_many_options[0].buy_limit; //当前购买限制数, 为0 时为无限制购买
        this.popStock = this.info.has_one_group.has_many_options[0].stock; //当前库存
        this.groupStock = this.info.has_one_group.has_many_options[0].stock; //当前库存
      }
    },
    ...
    clickOpenGroup() {
      ...
      if (this.btnStatus) {
        ...
        //去开团
        this.$router.push(
          this.fun.getUrl("GroupGoods", { id: that.activityId })
        );
    ...
    setGroupInfo(_id) {
      ...
      for (
        let i = 0;
        i < this.info.has_one_group.has_many_options.length;
        i++
      ) {
        let _optionSpecs = this.info.has_one_group.has_many_options[i]
          .option_id;
        //判断是否包含
        if (_optionSpecs == optionsId) {
          // options.push(this.goodsInfo.has_many_options[i]);

          that.selectGroupInfo =
            that.info.has_one_group.has_many_options[i].has_many_option_level;

          optionsMaxCount =
            that.info.has_one_group.has_many_options[i].buy_limit; //当前组合的购买限制数, 为0 时为无限制购买
    ...
    selectGroup(_level_id) {
      ...
      if (this.goodsInfo.has_option == 1) {
        //有规格
        var selectGroupLever = that.selectGroupInfo.filter(function(v) {
        ...
        //无规格
        that.selectGroupInfo =
          that.info.has_one_group.has_many_options[0].has_many_option_level;
        var selectGroupLever = that.selectGroupInfo.filter(function(v) {
          if (v.level_id == that.level_id) {
    ...
    share(data) {
      let that = this;
      wx.ready(function() {
        let _titles = that.info.has_one_group.desc_title;

        let _links = that.info.share_url;

        let _imgUrls = that.info.has_one_group.has_one_goods.thumb;
        let _descs = that.info.has_one_group.desc_content;
    ...
    gotoGoods() {
      this.$router.push(this.fun.getUrl("GroupGoods", { id: this.activityId }));
    },
    //分享
    shareWeixin() {
      //不是微信端 不访问
    ...
    appShare(data) {
      ...
      let _imgUrl = that.info.has_one_goods.thumb;
