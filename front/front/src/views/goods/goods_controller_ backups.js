...
export default {
  ...
  methods: {
    ...
    loadBottom() {
      ...
      this.$router.push({
        name: "goodstab",
        params: { id: this.$route.params.id, goods: this.goodsInfo }
      });
    ...
    getData() {
      let that = this;
      $http.get("goods.goods.get-goods", { id: goods_id }, "加载中...").then(
        function(response) {
    ...
    isFavorite() {
      let that = this;
      $http
        .get("member.member-favorite.isFavorite", { goods_id: goods_id })
        .then(
          function(response) {
            console.log(response);
            if (response.result == 1) {
    ...
    removeFavorite() {
      ...
      $http
        .get(
          "member.member-favorite.destroy",
          { goods_id: goods_id },
          "处理中..."
        )
        .then(
    ...
    footSet(_goods_id) {
      ...
      $http
        .get("member.member-history.store", {
          goods_id: _goods_id,
    ...
    initShare() {
      ...
      let json = { url: _url };
      $http.get("member.member.wxJsSdkConfig", json).then(
        function(response) {
          console.log(response);
          if (response.result == 1) {
    ...
    share(data) {
      ...
      wx.ready(function() {
        let _title = that.fun.isTextEmpty(
        ...
        let _imgUrl = that.fun.isTextEmpty(
          that.goodsInfo.has_one_share.share_thumb
        )
          ? that.goodsInfo.thumb
          : that.goodsInfo.has_one_share.share_thumb;
        let _desc = that.fun.isTextEmpty(
