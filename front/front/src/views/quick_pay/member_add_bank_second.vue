<template>
	<div id="member_add_bank_second">
			...
			<div class="content">
				...
				<div class="item">
					...
					<div class="item" v-if="radio1==1">
							...
							<input type="text" disabled="true" placeholder="请选择有效期" v-model="date"/>
					...
					<div class="item"  v-if="bank.length<=0">
							...
							<input type="number"  placeholder="请输入证件号" v-model="id_no"/>
					</div>
					<div class="item">
		...
		>
					...
					<div class="content">
							...
							<van-radio-group v-model="radio" checked-color="#ff2c29"> 
								<li v-for="(item,index,key) in datas" :key="index" class="radio">
									<span>{{item.text}}</span>
		...
		>
					...
					<div class="content">
							...
							<van-radio-group v-model="radio1" checked-color="#ff2c29"> 
								<li v-for="(item,index,key) in card" :key="index" class="radio">
									<span>{{item.text}}</span>
									<van-radio :name="item.id"></van-radio>
								</li>
							</van-radio-group>
		...
		>
					...
					<div class="content">
							...
							<van-radio-group v-model="radio2" checked-color="#ff2c29"> 
								<li v-for="(item,index,key) in idType" :key="index" class="radio">
									<span>{{item.text}}</span>
									<van-radio :name="item.id"></van-radio>
								</li>
							</van-radio-group>
		...
		>
			...
			<yd-datetime type="month" v-model="date" ref="datetime" style="visibility: hidden;" :callback="shijian"></yd-datetime>
		</div>
	</template>
<script>
