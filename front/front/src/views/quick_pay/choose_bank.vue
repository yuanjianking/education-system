...
<div id="choose_bank">
		...
		<div class="content">
			<div class="bank-list">
				<div class="bank" v-for="(item,index,key) in bank" :key="index"   :class="getIconUrl(item)" @click.stop="toPay(item)">
					<div class="top">
						<div class="avator">
		...
		<yd-popup v-model="show1" position="bottom" width="100%"  height="50%">
					<div class="title">
						付款详情
						<i class="iconfont icon-close11" @click.stop="show1=false"></i>
					</div>
					<div class="pay-content">
						<div class="item"><span>支付金额</span><span style="color: #ff2c29;">￥{{amount}}</span></div>
						<div class="item"><span>支付流水号</span><span style="color: #666666;">{{pay_sn}}</span></div>
						<div class="item"><span>支付金额</span><span style="color: #666666;">{{bank_name}}</span></div>
						<div class="item"><span>验证码</span><input type="number" placeholder="请输入验证码" v-model="code" style="outline: none;border: none;text-align: right;"></input>	<yd-sendcode slot="right" v-model="start1" @click.native="verificationCode" type="danger"></yd-sendcode></div>
						<div style="text-align: left;color:#666;font-size:12px;" class="item" v-if="start1">已向手机{{payer_phone.substr(0,3)+"****"+payer_phone.substr(7)}}发送短信</div>
