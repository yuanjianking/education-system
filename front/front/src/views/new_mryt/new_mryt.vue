<template>
  <div id="new_mryt">
    ...
    <div id="content">
      ...
      <div class="list_info">
        <ul v-for="(item,index) in  level"
            :key="index">
          <li>{{item.reward_amount||0}}<span>{{item.status==1?'元':'%'}}</span></li>
          <li>{{item.reward_name}}{{item.status==2?'比例':''}}</li>
        </ul>
      </div>
      <div class="reward_item">
        <ul class="reward">
          <li>累计总奖励</li>
          <li>{{member.member_account}}<span>元</span></li>
        </ul>
        <div class="item_box">
          <ul v-for="(item,i) in reward"
              @click="toReward(item)"
              :key='i'>
            <i class="img"
               :class="item.icon"></i>
      ...
      <div class="team_date">
        ...
        <div class="list_info">
          <ul @click="toDirect">
            <li>{{direct_cost_count}}<span>人</span></li>
            <li>直推会员数量</li>
          </ul>
          <ul @click="toAll">
            <li>{{team_cost_count}}<span>人</span></li>
            <li>团队会员数量</li>
          </ul>
          <ul v-for="(item,i) in teamData"
              @click="gotoTeamDataCount(item)"
              :key='i'>
          ...
          <!-- <ul v-if="!fun.isTextEmpty(team_cost_count)">
...
<script>
import new_mryt_controller from "./new_mryt_controller";
export default new_mryt_controller;
</script>
