          ...
          <ul class="good_list">
            <li class="good_child" v-for="(item, index) in goods" :key="index">
              ...
              <div class="g_right">
                ...
                <div class="g_bottom">
                  <div class="money"><em>￥</em>{{item.price}}</div>
                  <div class="g_btn" v-show="now_goods != item.id" @click="setActiveGood(item.id)">设置当前讲解</div>
                  <div class="g_btn active_goods" v-show="now_goods == item.id" @click="cancelActiveGood(item.id)">取消当前讲解</div>
                </div>
              </div>
            </li>
          </ul>
              ...
              <template v-if="!fun.isTextEmpty(item.text)">
                ...
                <div class="more_operat" @click="activeIndex = index">
                  ...
                  <div class="chat_btn" v-if="activeIndex == index">
                    <div @click="clickBan(item)"><i class="iconfont icon-all_stopcommentsl"></i> 禁言</div><em style="padding:0 0.25rem;">|</em><div @click="clickOut(item)"><i class="iconfont icon-all_tiren"></i> 踢人</div>
                  </div>
                </div>
              </template>
              <template v-if="fun.isTextEmpty(item.text)">
...
            })
        ...
        message.forEach(function(val, index) {
          ...
          if (conversationType == TIM.TYPES.CONV_GROUP) {
            ...
            if (data.from == "administrator") {
              if (data.payload.extension == 'welcome') { //进入直播间自动回复
    ...
    getData(){
      let that = this;
      $http
        .post('plugin.room.frontend.anchor-aide.room',{id:this.$route.params.id},"").then(response => {
          if (response.result === 1) {
    ...
    setActiveGood(_id){
      ...
      // timer = null;
      $http
        .post('plugin.room.frontend.anchor-aide.explain-goods',{id:this.$route.params.id,goods_id:_id},"").then(response => {
          if (response.result === 1) {
            Toast("设置成功！");
            this.now_goods = _id;
          } else {
    ...
    cancelActiveGood(_id,_set){
      let that = this;
      $http
        .post('plugin.room.frontend.anchor-aide.cancel-explain',{id:this.$route.params.id,goods_id:_id},{}).then(response => {
          if (response.result === 1) {
            this.now_goods = null;
            // if(_set){
            //   timer = setTimeout(() => {
    ...
    getOther(name){
      ...
      $http
        .post(_url,{id:this.$route.params.id,page:1}).then(response => {
          if (response.result === 1) {
            if(name ==2){
    ...
    bannedMember(_mid,cancel){
      ...
      $http
        .post(_url,{id:this.$route.params.id,member_id:_mid}).then(response => {
          if (response.result === 1) {
