    ...
    <div id="content">
      <div class="top">
        <ul class="box">
          ...
          <li>当前{{name === 'buy'?lang.buy_name:lang.subscription_name}}价：¥{{buying.price}}/单位</li>
        </ul>
      </div>
      <div class="number">
      ...
      <div class="discount" v-if="order_data.order_deductions">
        <ul class="list">
          <li v-for="(item,i) in order_data.order_deductions" :key='i'>
            <span>可用{{item.coin}}{{item.name?item.name:'积分'}}抵扣{{item.amount}}元</span>
            <mt-switch v-model="item.checked" @change="discountHandle(order_data,item)"></mt-switch>
          </li>
        </ul>
      </div>
      <div class="sum">
        <ul class="box">
          ...
          <template v-if="order_data.order_discounts">
            <li v-for="(item,i) in order_data.order_discounts" :key='i'>
              ...
              <span v-if="Number(item.amount)>0">￥ -{{item.amount}}</span>
            </li>
          </template>

          <li v-if="order_data.order_deductions">
            <span>总抵扣</span>
            <span>￥-{{deduction_price || 0}}</span>
          </li>
          <li>
            <span>总金额</span>
            <span>¥{{goods_price || 0}}</span>
          </li>
        </ul>
      </div>
      <div class="sub_btn">
        <p>合计：<span><em class="small">￥</em>{{price || 0}}</span></p>
