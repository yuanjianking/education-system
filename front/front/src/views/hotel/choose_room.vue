<template>
  <div id="choose_room">
    ...
    <!-- <div id="content">
      ...
      <div class="content_a">
        <h1>
          {{ name }}<span>{{ catagory_name }}</span>
        </h1>
        <ul class="time">
          <li class="time_a" @click="show2 = true">
            {{ fun.isTextEmpty(start) ? "请选择入住日期" : start
            }}<span>入住</span>
          </li>
          <li class="time_b" v-if="!fun.isTextEmpty(end)">
            <span>{{ night_day }}晚</span>
          </li>
          <li class="time_c" @click="show3 = true">
        ...
        >
          <div class="type_box">
            <div class="room_box" v-for="item in room_arr">
              <div class="img">
                <img :src="item.thumb" />
              </div>
              <ul class="info_box">
                ...
                <li class="info_b">
                  <p class="price">
                    <span>¥</span> <font>{{ item.price }}</font
                    >起
                  </p>
                  <button type="button" @click="gotoGoods(item.id)">
                    预定
                  </button>
                </li>
              </ul>
