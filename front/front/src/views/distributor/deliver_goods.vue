  ...
  <div>
    ...
    <ul class="goods-box" style="padding-top: 0.5rem">
      <van-checkbox-group v-model="selectCheck" ref="checkboxGroup">
        <van-checkbox :name="item" checked-color="#f01818" class="border_1px" @click="toggleAll" icon-size='15px' v-for="(item, index) in list" :key="index">
          <li>
            <div class="left">
              <img
            ...
            <div class="right">
              ...
              <p class="stock">进货价:<em style="color:#f01818;">￥</em><span class="price">{{item.goods?item.goods.price:''}}</span> <span class="repertory">库存 {{item.stock}}</span></p>
            </div>
          </li>
        </van-checkbox>
      </van-checkbox-group>
    ...
    <div class="car-btn" :style="{width: fun.getPhoneEnv() == 3 ?'375px':''}" :class="[fun.getPhoneEnv() == 3 ? 'pcStyle':'']">
      ...
      <div class="buy" :style="{background:(!fun.isTextEmpty(selectCheck)?'#f74c4c':'#999999')}" @click="addOrder">下单</div>
    </div>
  </div>
</template>
