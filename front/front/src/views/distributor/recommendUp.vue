...
<template>
  <div id="recommendUp">
    ...
    <div class="updateBox">
      ...
      <div class="btn" v-if="member.is_recommend" @click="gotoSubmit">去推荐</div>
    </div>
    <div class="main">
        ...
        <li v-for="(item, index) in list" :key="index">
          ...
          <div class="title_1 titleflex_1">
            <div style="display: flex;">
              <div class="member_name">{{item.child_member?item.child_member.nickname:''}}</div><div class="member_id">{{item.id}}</div>
            </div>
            <div>23434324323</div>
          </div>
          <div class="title_2" style="display:flex;flex-direction:column;">
            ...
            <div style="font-size:0.75rem;width:100%;">{{item.created_at.substr(5,7)}}/{{item.created_at.substr(8,10)}}</div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getData(){
      $http
      .post('plugin.channel.frontend.recommend.center',{},"").then(response => {
        if (response.result === 1) {
    ...
    getMoreData() {
      ...
      if (this.page >= this.total_page) {
        ...
        $http.get(_url,{page:this.page},"").then((response)=> {
