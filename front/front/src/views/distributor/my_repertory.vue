  ...
  <div>
    ...
    <ul class="goods-box">
      <li v-for="(item, index) in list" :key="index">
        <div class="left">
          <img
            :src="item.goods?item.goods.thumb:''"
            alt="">
        </div>
        <div class="right">
          <p class="title">{{item.title}}</p>
          <span class="spec text-over">规格:{{item.goods_option?item.goods_option.title:''}}</span>
          <p class="stock">进货价:<span class="price">￥{{item.show_price}}</span> <span class="repertory">库存{{item.stock}}</span></p>
          <div class="ticket">
            <p class="number text-over">兑换券 {{item.goods_change?item.goods_change.certificate:'0'}}张</p>
            <p @click="showPop(item.stock,item.id,item.goods_change.certificate)" class="exchange" v-if="item.is_exchange">兑换券</p>
          </div>
        </div>
      </li>
    </ul>
    ...
    <van-popup v-model="show1" position="bottom" closeable close-icon="close">
      <div class="bottom-popop-box">
        ...
        <div class="stock-box">
          ...
          <p>可得到<span class="red">{{canExchangeNum}}</span>张兑换券</p>
        </div>
        <div class="remain">
          <p style="flex: 1;line-height: 2.5rem;">剩余库存：{{exchangeStock}}</p>
          <p class="sure-btn" @click="goodsExchange">{{exchangeStatus?"兑换成功，去换货":'确定'}}</p>
        </div>
      </div>
    </van-popup>
  </div>
