<template>
  <div id="CardOpenMembershipList">
    ...
    <div class="vip_box">
        <div class="img">
            <img :src="member_level.has_one_universal_level.thumb_image">
        </div>
        <ul class="text">
...
export default {
  ...
  activated() {
    if(this.fun.isTextEmpty(this.$route.params.level_id)){
      this.$router.push(
        this.fun.getUrl("CardOpenMembership", {
  ...
  methods:{
    ...
    openSubmit() {
      ...
      // if (!this.fun.isTextEmpty(this.member_level.validity) && this.member_level.validity > 0) {
      //   this.$router.push(
...
#CardOpenMembershipList{
  ...
  .btn{
    ...
    background-color: rgba(255, 255, 255, 1);
	  box-shadow: 0rem 0rem 0.56rem 0rem rgba(169, 169, 169, 0.24);
    position: fixed;
    bottom: 0;
