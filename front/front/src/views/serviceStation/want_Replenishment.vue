  ...
  <div id="replenishment">
    ...
    <div id="content">
      ...
      <div class="goods">
        <div class="classification">
          <ul v-if="!showClassification"
              @click="clickShowClassification">
            <li class="secondary">当前分类：&nbsp;&nbsp;<span class="nowSelect">{{selectCategoryName}}&nbsp;</span></li>
          ...
          <transition name="classificationFade">
            <div v-if="showClassification"
                 class="classificationMain"
                 id="ContetH">
              <p style="text-align:left;font-size:16px;"
                 @click="clickShowClassification">全部分类<span style="float:right;font-size:18px;padding-right:1rem;"><i class="fa fa-angle-up"></i></span></p>
              <div class="listClass">
                ...
                <p>二级分类</p>
                <dl>
                  <dt v-for="(SecondaryList, index) in this.saveChooseCategory.childrens"
                      :key="index"
                      @click.stop="chooseChildCategory(SecondaryList.id, saveChooseCategory.id, SecondaryList)"
                      :class="SecondaryList.id === nowID ? 'addClass' : 'removeClass'">{{SecondaryList.name}}</dt>
                </dl>
                <template v-if="this.cat_level == '3'&&!fun.isTextEmpty(tertiaryList)">
                  <p>三级分类</p>
                  <dl>
                    <dt v-for="(child, index) in tertiaryList.childrens"
                        :key="index"
                        :class="child.id === thridID ? 'addClass' : 'removeClass'"
                        @click.stop="chooseThirdCategory(child.id, saveChooseCategory.id,child.name)">{{child.name}}</dt>
                  </dl>
                </template>
              </div>
            </div>
        ...
        <div ref="goodWrapper"
             style="max-height: 36rem;;overflow:hidden;padding-left: 0.75rem;">
                ...
                <ul class="info-class">
                  ...
                  <li class="info_c">
                    ...
                    <div class="select"
                      ...
                      <transition name="fade">
                        <i @click.stop="
                          ...
                          "
                           ...
                           class="iconfont icon-life-game-sign"></i>
                      </transition>
                      <input type="tel"
                             v-if="
                      ...
                      <i @click.stop="addGood(good)"
                         class="iconfont icon-life-game-plus"></i>
                    </div>
                    <div class="select optionClass"
                         v-if="good.has_option == 1">
                      <p @click.stop="addGood(good)">可选规格</p><i v-if="!fun.isTextEmpty(goodsCartsTotal[good.id])">{{goodsCartsTotal[good.id].total}}</i>
                    </div>
                  </li>
                </ul>
              </li>
        ...
        <transition name="coverFade">
          <div class="cover-layer"
               ...
               @click="clickShowClassification"></div>
        </transition>
        <!-- 选规格弹窗 -->
        <yd-popup v-model="show1"
          ...
          <div class="standard-box">
            ...
            <ul class="goods-standard">
              <li class="size"
                  v-for="(spec,i) in goodsInfo.has_many_specs" :key='i'>
                <h1>{{ spec.title }}</h1>
                <div class="choice">
                  <span @click.stop="selectSpecs(item, item.id)"
                        v-for="(item,i) in spec.specitem" :key='i'
                        :class="specid.indexOf(item.id) >= 0 ? 'current' : ''">{{ item.title }}</span>
                </div>
              </li>
            </ul>
            <div class="number">
              <span>购买数量：</span>
              <div class="select">
                <i @click.stop="reduceGoods"
                   class="iconfont icon-life-game-sign"></i>
                <input type="tel"
                       title="数量"
                       v-model="goodsCount" />
                <i @click.stop="addGoods"
                   class="iconfont icon-life-game-plus"></i>
              </div>
            </div>
            <div class="bnt"
                 @click="submitAction">确定</div>
          </div>
        </yd-popup>
      </div>
    </div>
    ...
    <div class="a2q"
      ...
      <a href="javascript:void 0;"
        ...
        <i v-show="carts.length > 0"
           class="a2s">{{ cartsNum }}</i>
      </a>
      <div class="a2t"
           :style="leftPrice"
           pricearea="">
        <div v-show="carts.length > 0"
             class="">¥{{ cartsTotal }}</div>
        <div v-show="carts.length === 0"
             class="a2u">购物车是空的</div>
      </div>
      <a class="a2v"
         :class="{ disabled: goodsCarts.length === 0 }"
         @click="goodsOrder(goodsCarts)">选好了</a>
      <!-- 购物车弹窗 -->
              ...
              <div ref="cartWrapper"
                ...
                <ul class="minicart-goods-list single">
                  <li class="a3o single"
                    ...
                    <span class="a4b a3p checked"
                          @click.stop="delItemByCart(item.id)"></span>
                    <a class="a3s">
                      <table class="a3t">
                        <tbody>
                          <tr v-if="item.goods">
                    ...
                    <a style=""
                       ...
                       class="a3v"></a>
                    <em style=""
                        class="a3y">{{ item.total }}</em>
                    <a @click.stop="updateCart(item.id, 1)"
                       class="a3w"></a>
                  </li>
                </ul>
              </div>
              <div class="a3n single"
                   style=""></div>
            </div>
          </div>
        </div>
