    ...
    <div id="list_box">
      ...
      <div class="list">
        <ul class="item" v-for="(item,i) in earningList" :key='i'>
          ...
          <li>{{item.amount}}</li>
          <li>
            <yd-button type="hollow" @click.native="getAdvertising(item.id)">领取</yd-button>
          </li>
        </ul>
        <!-- 领取弹窗 -->
        <yd-popup v-model="show1" position="center" height="100%" width="100%">
          <div class="bg_img">
            <img class="adv_img"
                 :src="adv_thumb"
                 alt="图片">
            <div class="bg_color">
              <div class="receive">
                <ul class="text">
                  <li>已成功领取分销佣金</li>
                  <li>{{income}}元</li>
                  <li>已存入{{income_name_text}}</li>
                </ul>
                <yd-button type="hollow" @click.native="toAdvertising">我知道了</yd-button>
              </div>
            </div>
          </div>
        </yd-popup>
