    ...
    <div class="main">
      ...
      <div class="items" v-if="chooseStore.id">
        <div class="title-box">
          ...
          <p class="mr"><span class="rest" v-if="isRest == 0" @click="changeDayStatus('rest')">休息</span></p>
          <p class="mr"><span class="add" v-if="isRest == 1" @click="changeDayStatus()">上班</span></p>
          <p><span class="add" @click="addTime">添加工作时间</span></p>
        </div>

        <div class="time-box">
          <div class="time-item" v-for="item in day_time" :key="item.id">
            <p class="left">{{item.work_hour_start}}-{{item.work_hour_end}}</p>
            <div class="right">
              <p class="mr"><span class="rest" @click="changeStatus(item,'rest')" v-if="item.status==0">休息</span></p>
              <p class="mr"><span class="add" @click="changeStatus(item,)" v-if="item.status==1">上班</span></p>
              <p><span class="edit" @click="editTime(item)">编辑</span></p>
            </div>
          </div>
        </div>
...
<style lang="scss" rel="stylesheet/scss" scoped>
  #work_time {
    ...
    .time-item {
      ...
      margin-top: 0.5rem;
      padding: 1rem 0.5rem;
      box-shadow: 0px 0px 9px 0px rgba(169, 169, 169, 0.24);
      border-radius: 3px;
    }
    .left {
      flex: 1;
