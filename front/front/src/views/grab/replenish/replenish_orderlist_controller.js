...
export default {
  ...
  methods: {
    toDetail(id) {
      this.$router.push(this.fun.getUrl('ReplenishOrderDetail', {}, {id: id, name: 'replenish'}));
    },
    //点击按钮的操作
    operation(btn, order) {
    ...
    checkStatus(status) {
      if (status === '1') {
        this.api = 'plugin.kingtimes.frontend.provider.distributorOrderList.waitSend';
      } else if (status === '2') {
        this.api = 'plugin.kingtimes.frontend.provider.distributorOrderList.waitReceive';
      } else if (status === '3') {
        this.api = 'plugin.kingtimes.frontend.provider.distributorOrderList.completed';
      } else {
        this.api = 'plugin.kingtimes.frontend.provider.distributorOrderList.index';
      }
    },
    //获取数据
    getData(status) {
