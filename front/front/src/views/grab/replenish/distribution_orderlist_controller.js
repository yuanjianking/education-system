...
export default {
  ...
  methods: {
    toDetail(id) {
      this.$router.push(this.fun.getUrl('ReplenishOrderDetail', {}, {id: id, name: 'distribution'}));
    },
    //点击按钮的操作
    operation(btn, order) {
      if (btn.value === 8) {
    ...
    // 确认发货
    send(order, btn) {
      this.$router.push(this.fun.getUrl('Send', {id: order.id}));
    },
    ...
    checkStatus(status) {
      if (status === '1') {
        this.api = 'plugin.kingtimes.frontend.providerOrder.list.waitSend';
      } else if (status === '2') {
        this.api = 'plugin.kingtimes.frontend.providerOrder.list.waitReceive';
      } else if (status === '3') {
        this.api = 'plugin.kingtimes.frontend.providerOrder.list.completed';
      } else {
        this.api = 'plugin.kingtimes.frontend.providerOrder.list.index';
      }
    },
    //获取数据
    getData(status) {
