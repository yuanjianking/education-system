  ...
  methods: {
    ...
    goodsOrder(val) {
      ...
      if (val.length === 0 || this.carts.length === 0) {
        ...
        this.$router.push(this.fun.getUrl('DistributionOrderDetail', {cartIDs: cartIDs}, {name: 'replenish'}));
      }
    },
    ...
    addCartRequest(_goodsId, _optionsId, _total) {
      ...
      $http.get("plugin.kingtimes.frontend.providerCart.ProviderCart.store", json, "添加中...").then(response => {
    ...
    inputChange(id, num) {
      ...
      $http.get('plugin.kingtimes.frontend.providerCart.ProviderCart.updateNumV2', {
        id: id,
        num: num,
      }).then((response) => {
    ...
    updateCart(id, num, good) {
      ...
      $http.get('plugin.kingtimes.frontend.providerCart.ProviderCart.updateNum', {
        id: id,
        num: num,
      }).then((response) => {
        // console.log('result', response);
    ...
    delItemByCart(cart_id) {
      ...
      $http.get('plugin.kingtimes.frontend.providerCart.ProviderCart.destroy', {
        ids: cart_id
      }).then((response) => {
        if (response.result === 1) {
          that.getCart();
    ...
    getCart(goods) {
      ...
      that.cartsNum = 0;
      that.cartsTotal = 0;
      $http.get('plugin.kingtimes.frontend.providerCart.ProviderCart.index', {}).then((response) => {
        if (response.result === 1) {
        ...
        if (response.result === 1) {
          ...
          that.$nextTick(() => {
            ...
            if (!that.goodScroll) { //判断是否已经初始化
              that._initScrollG();
              // 计算最大向上滚动距离
              console.log(that.goodScroll.maxScrollY);
            } else {
