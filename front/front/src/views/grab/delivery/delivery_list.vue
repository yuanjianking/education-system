<template>
  <div id="delivery_order">
    <c-title :hide="false" text='配送订单'></c-title>
    <!--配送站的配送订单-->
      ...
      <!--<el-tabs v-model="activeName" @tab-click="getRebate">-->
        ...
        <!--<el-tab-pane label="全部" name="-1">-->
          <div class="order" v-if="!status">
            <div class="goods-list" v-for="(good,index) in list" :key='index' @click="toRobbingDetail(good.id)">
              <h1>订单号：{{good.order_sn}}<span>{{good.status_name}}</span></h1>
              <div class="goods" v-for="(item,i) in good.has_many_order_goods" :key='i'>
                <div class="img">
                  <img :src="item.thumb">
                </div>
                <ul class="info">
              ...
              <div class="btn" v-if="good.button_models && good.button_models.length > 0">
                <span :key='i' v-for="(btn,i) in good.button_models" @click.stop="operation(btn,good)">{{btn.name}}</span>
              </div>
            </div>
          </div>
        <!--</el-tab-pane>-->
        ...
        <!--<el-tab-pane label="待发货" name="1">-->
          <div class="order" v-if="status === '1'">
            <div class="goods-list" v-for="(good,index) in list" @click="toRobbingDetail(good.id)" :key="index">
              <h1>订单号：{{good.order_sn}}<span>{{good.status_name}}</span></h1>
              <div class="goods" v-for="(item,i) in good.has_many_order_goods" :key='i'>
                <div class="img">
                  <img :src="item.thumb">
              ...
              <div class="btn" v-if="good.button_models && good.button_models.length > 0">
                <span v-for="btn in good.button_models" :key="btn" @click.stop="operation(btn,good)">{{btn.name}}</span>
              </div>
            </div>
          </div>
        <!--</el-tab-pane>-->
        ...
        <!--<el-tab-pane label="已完成" name="3">-->
          <div class="order" v-if="status === '3'">
            <div class="goods-list" v-for="(good,index) in list" @click="toRobbingDetail(good.id)" :key="index">
              <h1>订单号：{{good.order_sn}}<span>{{good.status_name}}</span></h1>
              <div class="goods" v-for="item in good.has_many_order_goods" :key="item">
                <div class="img">
                  <img :src="item.thumb">
                </div>
                <ul class="info">
...
<script>
  import delivery_list_controller from './delivery_list_controller';

  export default delivery_list_controller;
</script>
