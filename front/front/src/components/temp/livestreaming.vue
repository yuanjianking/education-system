<!-- 带货直播 首页装修页组件 -->
<template>
  <div id="livestreaming" :style="{ backgroundColor: datas.params.livebgcolor }">
    <van-tabs v-model="active" swipeable @change="getData" :background="datas.params.bgcolor" :title-active-color="datas.params.selectedcolor" :color="datas.params.selectedcolor" :title-inactive-color="datas.params.wordcolor">
      <van-tab title="推荐">
        <template v-if="active == 0&&!fun.isTextEmpty(recordsList)">
      ...
      <van-tab title="直播中">
        <template v-if="active == 1&&!fun.isTextEmpty(recordsList)">
          <div class="living-box">
            <ul ref="newArray1"
              class="zq-waterfall-left living-item">
      ...
      <van-tab title="预告">
        <template v-if="active == 2&&!fun.isTextEmpty(recordsList)">
          <div class="living-box">
            <ul ref="newArray1"
              class="zq-waterfall-left living-item">
      ...
      <van-tab title="精彩回放" v-if="isShowBack">
        <template v-if="active == 3&&!fun.isTextEmpty(recordsList)">
          <div class="living-box">
            <ul ref="newArray1"
              class="zq-waterfall-left living-item">
    ...
    <router-link v-if="!isloading" style="padding:0 0 1rem 0;color:#999;"  tag="p" :to="fun.getUrl('liveList')">前往直播列表 ></router-link>
  </div>
</template>
    ...
    getData(){
      ...
      switch (that.active) {
        ...
        case 3:
          ...
          break;
        default:
           console.log("error::components/temp/livestreaming !!! name is Undefined",that.active)
          break;
      }
      $http
        .post(_url,_json,"").then(response => {
          ...
          if (response.result === 1) {
            ...
            if(!that.fun.isTextEmpty(that.datas.params.number)){
              that.recordsList = that.recordsList.slice(0,that.datas.params.number);
            }
            that.newArray1 = [];
            that.newArray2 = [];
