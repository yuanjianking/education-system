  ...
  <div :style="'background:' + datas.params.bgcolor">
    <div id="headline-box" @click="gotoList" v-if="datas.params.shownum == 2">
      ...
      >
        ...
        <van-swipe-item
          v-for="(item, i) in listData_2"
          :key="i"
          v-if="listData_2.length == 1"
        >
          <div
            class="li"
            v-for="(item_1, i) in item"
      ...
      <!--        :style="'height:3.75rem;width:85%;background:' + datas.params.bgcolor"-->
      <!--      >-->
      <!--        <yd-rollnotice-item v-for="item in datas.data">-->
      <!--          <span class="left"> 热议</span>-->
    ...
    <div id="headline" @click="gotoList" v-if="datas.params.shownum == 1">
      ...
      <!--        <yd-rollnotice-item v-for="item in datas.data"-->
      <!--          ><span class="left"> 热议</span-->
      <!--          ><span class="right">{{ item.title }}</span></yd-rollnotice-item-->
      <!--        >-->
      <!--      </yd-rollnotice>-->
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    if (Object.prototype.toString.call(this.datas.data) == "[object Object]") {
      ...
      Object.prototype.toString.call(this.datas.data) == "[object Array]"
    ) {
      if (this.datas.data) {
  ...
  methods: {
    gotoList() {
      localStorage.setItem("showheadline", JSON.stringify(this.datas));
