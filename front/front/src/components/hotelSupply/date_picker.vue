...
<!--              {{ day_item.value }}-->
<!--            </li>-->
            <div v-for="(day_item, day_index) in item.day">
              <li v-if="day_item.isOld == '0'" :key="day_index":class="{ choose1: day_index == d_index && index == m_index }" @click="chooseDate(index, day_index, day_item.all_date)">
              <span>{{ day_item.value }}</span
              ><br /><span>{{
                day_index == d_index && index == m_index ? type : ""
              }}</span><span v-if="(day_item.is_manfang&&day_item.is_manfang==1)">{{ day_index == d_index && index == m_index ? '' : "满房"}}</span><span v-if="(day_item.vprice&&day_item.is_manfang!=1)">{{day_index == d_index && index == m_index ? '' : day_item.vprice
              }}</span>
              </li>
              <li
                v-if="day_item.isOld == '-1'"
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      month_arr.forEach(item => {
        ...
        for (let i = 0; i < days; i++) {
          let obj = {};
          obj["all_date"] = item.full + `-${i + 1 < 10 ? `0${i + 1}` : `${i + 1}`}`;
        ...
        this.dayArr.forEach(items=>{
          if(items.month == item.month){
            for(let i=0;i<day_arr.length;i++){
            if(this.getTomorrow(this.dayArr[this.dayArr.length-1].date)===day_arr[i].all_date){
              day_arr[i].isOld = 1;
              }else if(day_arr[i].vprice ){
            ...
            for(let i=0;i<day_arr.length;i++){
              if(items.day==day_arr[i].value||(day_arr[i].lastValue&&items.day==day_arr[i].lastValue)){
                day_arr[i].is_manfang = items.is_manfang;
                day_arr[i].vprice = items.vprice;
                day_arr[i].isOld = 1;
              }
            }
          }else{
  ...
  mounted() {
    let dayArr = [];
    this.nextpricearr.forEach(item=>{
      let date = item.date;
