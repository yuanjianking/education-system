  ...
  <div id="orderList">
    <van-checkbox-group v-model="checkList" @change="multiplePayChange">
      <div
        ...
        :class="orderType == 'lease_toy' && index == datasource.length - 1
            ? 'Margintop shop' : 'shop'" :key="index"
      >
        <div class="title">
          <van-checkbox
        ...
        <router-link
          :to="fun.getUrl(detailUrl, { order_id: order.id, orderType: orderType })"
          ...
          <div class="goods">
            ...
            <div class="warp">
              ...
              <div class="price">
                <div class="money">
                  <small>￥</small>
                  {{ orderType == "lease_toy" ? good.price /
                  order.has_one_lease_toy_order.return_days /
                  good.total : good.price }}
                ...
                <div v-if="detailUrl == 'HotelOrderDetail'">
                  ×{{ getDays(order.hotel_order.enter_at,order.hotel_order.leave_at)}}晚{{ good.total }}间
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="allbt" v-if="order.button_models.length > 0||!fun.isTextEmpty(order.replenishment_button)">
          ...
          <template v-if="order.button_models.length > 0">
            <span
              v-for="(item, i) in order.button_models" :key="i"
    ...
    methods: {
      ...
      operation(btn, order) {
        ...
        if (btn.value == 8) {
          ...
          btn.value == "package_deliver_clerk"||btn.value == 144
        ) {
          // 申请退还
